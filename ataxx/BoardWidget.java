...
class BoardWidget extends Pad  {
    ...
    public synchronized void paintComponent(Graphics2D g) {
        ...
        g.drawRect(0, 0, SQDIM * SIDE, SQDIM * SIDE);
        g.setColor(LINE_COLOR);
        for (int x = 0; x <= SQDIM * SIDE; x += SQDIM) {
            g.drawLine(x, 0, x, SQDIM * SIDE);
            g.drawLine(0, x, SQDIM * SIDE, x);
        }
        for (int col = 'a'; col < (int) ('h'); col++) {
            for (int row = '1'; row < (int) ('8'); row++) {
                int index = Board.index((char) (col), (char) (row));
                if (_model.get(index) != EMPTY) {
                    PieceColor curr = _model.get(index);
                    int cx = row - ('a' - 1) * SQDIM;
                    int cy = row - ('1' - 1) * SQDIM;
                    if (curr == RED) {
    ...
    private void handleClick(String unused, MouseEvent where) {
        ...
        if (where.getButton() == MouseEvent.BUTTON1) {
            ...
            if (mouseCol >= 'a' && mouseCol <= 'g'
                ...
                if (_blockMode) {
                    String block = "block " + mouseCol + "" + mouseRow;
                    _commandQueue.offer(block); //fixme
                } else {
                    if (_selectedCol != 0) {
                        String move = _selectedCol + "" + _selectedRow;
